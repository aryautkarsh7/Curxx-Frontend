...
export async function loadFacilities(type: 'hospital' | 'clinic', params: SearchParams) {
  ...
  const data = await api.facilities(query).catch(() => ({ items: [], total: 0, page: 1, limit: 8, pages: 1, facets: { areas: [] } }));
  return { query, data };
}
